        ...
        if #available(watchOSApplicationExtension 7.0, *) {
            VStack {
                ...
                NavigationLink(isActive: $jump) {
                    TimerView(lapTiming: .init(0), playingActivityOut: $playingActivityOut, playingActivityIn: $playingActivityIn)
                } label: {
                    EmptyView()
                }
                .buttonStyle(.plain)
...
struct CircleProgressView_Previews: PreviewProvider {
    static var previews: some View {
        CircleProgressView(playingActivityOut: .constant(nil), playingActivityIn: .constant(nil))
    }
}
